...
#+begin_src emacs-lisp :tangle yes
  (use-package org-projectile
    ...
    :bind
        (:map projectile-command-map ("C-c p n" . org-projectile-capture-for-current-project))
    :config
    (push (org-projectile-project-todo-entry) org-capture-templates)
    :custom
    (org-projectile-capture-template (format "%s" "* TODO %?"))
    (org-projectile-projects-file (file+headline "~/org/master.org" "Projects")))
#+end_src
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package mu4e-alert
    ...
    (defun my/mu4e-alert-mode-line-refresh ()
      (interactive)
      (mu4e~proc-kill)
      (mu4e-alert-enable-mode-line-display))
