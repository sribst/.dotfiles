...
**** Agenda view =org-super-agenda=
 ...
 #+BEGIN_SRC emacs-lisp :tangle yes
       ...
       :init
       (setq org-super-agenda-mode nil)
       :custom
       (org-agenda-custom-commands
        '(("b" "Experimental"
            ...
            ((agenda ""
              ...
              '(
                (:name "To do ASAP" :scheduled t)
                (:name "do not miss" :deadline t)
                (:name "Information" :and (:time-grid t :todo nil))
                (:name "Today" :and (:time-grid t :todo t :date today))
                ;; (:name "next 2 day" :date t)
              ))))
             (alltodo ""
              ((org-super-agenda-groups
              '((:name "Current Work"  :and(:tag "work" :todo "INPROGRESS"))
               ...
               (:name "Current personal" :todo "INPROGRESS")
               (:name "Next personnal"   :todo "TODO")
               (:name "on pause Work"    :todo "WAITING")
               (:name "tobuy" :todo ("TOBUY" "BUY"))
...
"* TORESERVE %^{Name} [[google-maps:%\\3][Google Maps]] %^g
  ...
  :Place:    %^{Place}
  :With:      %^{With}
  :GMap:      [[google-maps:%\\2][Google Maps]]
  :END:
  %^{When}T
")

  (defvar my/org-togo-template
"* TOGO %^{Name} [[google-maps:%\\3][Google Maps]] %^g
  :PROPERTIES:
  :Created:     %U
  :Name:   \\1
  :Type:   %^{Type}p
